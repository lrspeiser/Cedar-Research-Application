...
def test_home_ok():
    ...
    # Home should include production layout CSS and client log hook
    assert re.search(r"\.two-col\s*\{[\s\S]*grid-template-columns", r.text)
    assert "/api/client-log" in r.text
